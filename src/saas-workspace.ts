export const WORKSPACE_ROLES = ['owner', 'mentor', 'student', 'client'] as const;
...
export function switchWorkspace(workspaceState, id, now = new Date().toISOString()) {
  if (!workspaceState.workspaces.some((workspace) => workspace.id === id)) throw new Error('Unknown workspace');
...
export function importWorkspaceBundle(config, raw) {
  ...
  if (bundle.product?.slug && bundle.product.slug !== config.slug) throw new Error('Workspace bundle slug mismatch');
  if (!bundle.workspaceState?.workspaces?.length) throw new Error('Invalid workspace bundle');
  return bundle;
}
